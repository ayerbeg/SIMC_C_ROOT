/**
 * HRSl.cpp - Complete implementation with all 32 aperture checks
...
bool HRSl::Transport(TrackState& track) {
    /**
     * Transport through HRSl spectrometer
    ...
    // Collimator dimensions (from mc_hrsl.f lines 53-65)
    ...
    const double h_exit = 3.335;   // horizontal half-width at exit (cm) - 0.5mm narrower than HRSr
    ...
    const double z_entr = 110.9;   // entrance z position (cm) - 0.9cm further than HRSr
    ...
    // Made it to the hut!
    ++stats_.lSTOP_hut;
    ++stats_.lSTOP_successes;
    return true;
}

bool HRSl::Reconstruct(const FocalPlaneState& fp, TargetState& target) {
    /**
...
bool HRSl::GetFocalPlane(const TrackState& track, FocalPlaneState& fp) const {
    // Extract focal plane coordinates
    // NOTE: HRSl uses dx/dy (slopes), NOT xp/yp (mrad)
...
bool HRSl::LoadMatrices(const std::string& forward_file, const std::string& recon_file) {
...
bool HRSl::ParseMatrixFile(const std::string& filepath, MatrixElements& matrices, bool is_reconstruction) {
    ...
    std::cout << "Loaded HRSl matrix file: " << filepath << std::endl;
    std::cout << "  Found " << matrices.n_classes << " transformation classes" << std::endl;
    
    return true;
}

void HRSl::TranspMatrix(TrackState& track, int class_num, double pathlen) {
    /**
...
void HRSl::Project(TrackState& track, double distance) {
    /**
