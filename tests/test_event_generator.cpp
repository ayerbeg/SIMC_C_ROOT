...
// Create standard test configuration matching EventGenerator.cpp structure
nlohmann::json create_test_config() {
    return nlohmann::json{
        ...
        {"generation", {
            {"reaction_type", "elastic"},  // Will map to ReactionType::ELASTIC
...
void test_constructor() {
    ...
    // CORRECTED: Use ReactionType::ELASTIC (not kHydElastic)
...
void test_hydrogen_elastic() {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (gen.GenerateEvent(event, main)) {
            ...
            if (n_success == 1) {
                ...
                std::cout << "    Q2        = " << event.Q2 / 1.0e6 << " GeV^2" << std::endl;
    ...
    if (n_success == 0) {
        std::cerr << "  ERROR: No events generated! Check implementation." << std::endl;
    }
...
// Test 4: Angle conversions - HMS at phi0=0° (modern config)
void test_angle_conversions_hms() {
    std::cout << "Test 4a: Angle Conversions - HMS at phi0=0°..." << std::endl;
    
    nlohmann::json config_json = create_test_config();
    config_json["spectrometer_hadron"]["phi"] = 0.0;  // HMS position
    ...
    double phi0 = 0.0 * kDegToRad;  // HMS at 0°
    ...
    std::cout << "  Error: Δxptar=" << (xptar_back - xptar_orig) 
              << ", Δyptar=" << (yptar_back - yptar_orig) << std::endl;
    
    // Check round-trip (should be machine precision)
    assert(approx_equal(xptar_orig, xptar_back, 1e-10));
    assert(approx_equal(yptar_orig, yptar_back, 1e-10));
    
    std::cout << "  PASSED (HMS at phi0=0° works perfectly!)" << std::endl;
}

// Test 4b: Angle conversions - SOS at phi0=90° (legacy config)
void test_angle_conversions_sos() {
    std::cout << "Test 4b: Angle Conversions - SOS at phi0=90°..." << std::endl;
    ...
    // Test round-trip conversion
    double theta0 = 12.5 * kDegToRad;
    double phi0 = 90.0 * kDegToRad;  // SOS at 90°
    ...
    std::cout << "  PASSED (SOS at phi0=90° also works perfectly!)" << std::endl;
}

// Test 5: Statistics tracking
void test_statistics() {
    std::cout << "Test 5: Statistics Tracking..." << std::endl;
...
int main() {
    ...
    std::cout << "CORRECTED VERSION" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    try {
        ...
        test_angle_conversions_hms();  // NEW: Tests HMS at phi0=0°
        test_angle_conversions_sos();  // Tests SOS at phi0=90°
        test_statistics();
        
        std::cout << "========================================" << std::endl;
        std::cout << "All tests PASSED!" << std::endl;
        std::cout << "Your C++ angle conversions work for" << std::endl;
        std::cout << "ALL spectrometer angles (not just ±90°)" << std::endl;
        std::cout << "========================================" << std::endl;
